...
entity channel_stats is
    ...
    port (
        ...
        frame_start : in  std_logic; -- tuser (SOF)
        frame_end   : in  std_logic; -- tlast (EOL)
...
architecture rtl of channel_stats is
    ...
    -- Entree  : valeur 32 bits
    -- Sortie  : racine approchee 16 bits
    -- =========================================================
    ...
    begin
        ...
        -- Estimation initiale : decalage de 8 bits
        x := shift_right(val, 8) + 1;

        -- 8 iterations Newton-Raphson : x = (x + val/x) / 2
        ...
        -- Retourner les 16 bits bas (la racine ne depasse pas 16 bits)
    ...
    -- sum_sq max = 640*480*3*255^2 = 60,111,948,000 ? 36 bits
    signal acc_sum_sq : unsigned(35 downto 0) := (others => '0');

    -- Compteur pixels (x3 canaux) max = 960*640*3 = 1,843,200 ? 24 bits suffisent
    signal pix_count  : unsigned(23 downto 0) := (others => '0');
...
begin
    ...
    -- =========================================================
    -- Process : Accumulation sur une frame complete
    -- =========================================================
    ...
    begin
        if rst_n = '0' then
            ...
            -- Defaut : done = 0 (pulse 1 cycle)
            done_reg <= '0';

            -- --------------------------------------------------
            -- Remise a zero en debut de frame (SOF)
            -- --------------------------------------------------
            ...
            if frame_end = '1' and pix_valid = '1' then
                ...
                if pix_count > 0 then
                    ...
                    -- E[X^2] = acc_sum_sq / pix_count
                    v_esq := acc_sum_sq / pix_count;

                    -- mean^2 (pour soustraire)
                    v_mean_sq := resize(v_mean8 * v_mean8, 28);

                    -- var = E[X^2] - mean^2   (clamp a 0)
                    ...
                    -- d'une image uint8 ne depasse pas 255^2=65025
